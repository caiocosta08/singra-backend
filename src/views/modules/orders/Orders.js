...
const Orders = () => {
  ...
  const getTable = async () => {
    ...
    Format.className = 'teste';
    await Promise.all(
      orders.map((o) => {
        console.log(o);
        let texto = document.createTextNode(o.client);
        Format.appendChild(texto);
      }),
    );
  };
  const Prints = () => (
  ...
  const reportHtml = React.useRef(null);
  ...
  const columns = [
    ...
    { field: 'client', headerName: 'Cliente', width: 180 },
    {
      ...
      headerName: 'Valor',
      width: 120,
      valueGetter: (params) => `${formatMoney(params.row.price)}`,
    },
    { field: 'payment_method', headerName: 'Pagamento', width: 120 },
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: 10, display: 'flex', width: 360 }}>
          ...
          {/* <CButton style={{ margin: 10 }} color="primary" onClick={print}>
            Export PDF <Add style={{ color: '#fff' }} />
          </CButton> */}
          <CButton
      ...
      {/* <Box sx={{ width: '100%' }} ref={reportHtml}>
        <DataGrid
